                ...
                <table class="table table-striped">
                            ...
                            <div class="modal-content">
                                <form action="" method="post">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="form-floating mb-3">
                                            <input type="date" name="tanggalmk" id="tmk" class="form-control" placeholder="Pilih Tanggal" required>
                                        ...
                                        <div class="form-floating mb-3">
                                            ...
                                            <label for="jumlahstok">
                                                Masukkan Jumlah
                                                <span id="perhitungan" class="ms-2"></span> <!-- Perhitungan di sebelah sini -->
                                            </label>
                                        </div>
                    ...
                    <script>
                        function lihatBarang(select) {
                            ...
                            const jumlahSatuan = parseInt(selectedOption.getAttribute('data-jumlah_satuan')) || 0;
                            const stok = parseInt(selectedOption.getAttribute('data-stok')) || 0;

                            document.getElementById('barangsatuan').value = `${satuan} (${jumlahSatuan})`;
                            document.getElementById('stok').value = stok;

                            // Menyimpan jumlah satuan di data attribute untuk digunakan dalam perhitungan
                            document.getElementById('barangmk').setAttribute('data-jumlah_satuan', jumlahSatuan);
                        ...
                        document.getElementById('jumlahstok').addEventListener('input', function() {
                            ...
                            const jumlahSatuan = parseInt(document.getElementById('barangmk').getAttribute('data-jumlah_satuan')) || 0;

                            const jenisTransaksi = document.getElementById('barangmk').value; 

                            // Validasi hanya diterapkan jika jenis transaksi adalah "barangkeluar"
                            if (jenisTransaksi === 'barangkeluar') {
                                // Hitung jumlah maksimum yang bisa dimasukkan berdasarkan total stok dan jumlah satuan
                                const maxJumlah = Math.floor(maxStok / jumlahSatuan); // Membatasi jumlah stok agar tidak melebihi total stok

                                if (jumlahInput > maxJumlah) {
                                    // Batasi input pengguna agar tidak melebihi total stok yang tersedia
                        ...
                        document.getElementById('barangmk').addEventListener('change', function() {
                            // Update perhitungan setelah jenis transaksi berubah
                            updatePerhitungan();
                        });

                        // Fungsi untuk memperbarui tampilan perhitungan
                        function updatePerhitungan() {
                            const jenisTransaksi = document.getElementById('barangmk').value;
                            const stok = parseInt(document.getElementById('stok').value) || 0;
                            const jumlahSatuan = parseInt(document.getElementById('barangmk').getAttribute('data-jumlah_satuan')) || 0;
                            const jumlahInput = parseInt(document.getElementById('jumlahstok').value) || 0;

                            let totalPerhitungan = jumlahInput * jumlahSatuan;
                            let perhitunganText = `${totalPerhitungan}`;

                            // Menentukan tanda + atau - berdasarkan jenis transaksi
                            if (jenisTransaksi === 'barangmasuk') {
                                perhitunganText = `+ ${perhitunganText}`;
                            } else if (jenisTransaksi === 'barangkeluar') {
                                perhitunganText = `- ${perhitunganText}`;
                            }

                            // Menampilkan hasil perhitungan di dalam span perhitungan
                            document.getElementById('perhitungan').innerText = `(${stok} ${jenisTransaksi === 'barangmasuk' ? '+' : '-'} ${totalPerhitungan} = `;

                            // Perhitungan hasil akhir (stok -/+ (jumlah yang diketik * jumlah satuan) = hasil)
                            let hasil = (jenisTransaksi === 'barangmasuk') 
                                ? stok + totalPerhitungan 
                                : stok - totalPerhitungan;

                            // Menambahkan hasil perhitungan
                            document.getElementById('perhitungan').innerText += `${hasil})`;
                            
                            // Perhitungan hasil akhir (stok -/+ (jumlah yang diketik * jumlah satuan) = hasil)
                            document.getElementById('jumlahstok').setAttribute('placeholder', 
                                `Masukkan Jumlah (${stok} ${jenisTransaksi === 'barangmasuk' ? '+' : '-'} ${totalPerhitungan} = ${hasil} )`);
                        }
                    </script>
